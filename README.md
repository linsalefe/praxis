...
- **Instrumentos** — catálogo clínico com escalas (PHQ-9, GAD-7, WHO-5, DASS-21, SRQ-20) e instrumentos qualitativos do paradigma: **Entrevista de Maastricht**, **WRAP**, **GAM** (Gestão Autônoma da Medicação) e **PTMF** (Poder, Ameaça, Significado). Preenchimento assistido, saída interpretada (rascunho editável) e finalização em **anexo PDF** vinculado ao paciente. O catálogo se sincroniza no boot (`upsert_catalogo`), sem migração. **GAM:** apoio à decisão compartilhada — a saída nunca sugere conduta de medicação (o psicólogo não prescreve). **PTMF:** formulação narrativa não-diagnóstica.
...
- **Telessessão (vídeo)** — para sessões `online`, uma **sala de vídeo** por sessão integrando um provedor pronto (**Jitsi** por default, sem chave de API; provider-agnóstico no backend). O nome da sala é **derivado por HMAC do UUID da sessão** — não-adivinhável, sem PII na URL, idempotente. O paciente entra pelo **link compartilhável** (sem portal/cadastro). **Gate de consentimento (CFP):** a sala só é liberada com consentimento `teleatendimento` registrado (Res. CFP nº 09/2024, que revogou a 11/2018 e a 04/2020 — *o profissional confirma a norma vigente*). Env: `PRAXIS_VIDEO_PROVIDER`, `PRAXIS_JITSI_BASE`.
...
- **Conformidade IA-CFP (Nota de Posicionamento do CFP sobre IA, jul/2025)** — não existe resolução específica de IA; a orientação vem da Nota de Posicionamento e da Cartilha 2025 (a Res. CFP 09/2024 rege o teleatendimento, não a IA). Transparência operacionalizada, reusando o que já existe: (1) **TCLE de uso de IA** versionado, registrável via `consentimentos` (`tipo="uso_ia"`, o texto versionado fica em `texto_aceito`) — o texto **declara a transferência internacional** (áudio + trechos processados por sub-processador nos EUA/OpenAI, base legal consentimento); **enforçado** nos fluxos de IA por paciente (Scribe, Sofia, Preparação, Instrumentos → 403 sem `uso_ia` ativo); (2) **log de uso de IA por paciente** derivado do `audit_log` (eventos reais e factuais de Sofia/Scribe/Preparação/Instrumento, marcados com `meta.paciente_id`) em `GET /pacientes/{id}/ia-log`, **incluído no pacote LGPD** (`export.json` + `resumo.pdf`); (3) página **pública** `/como-usamos-ia` explicando, de forma clara e verdadeira, o que a IA faz (apoio/rascunho, supervisão humana) e o que **não** faz (não decide, não substitui julgamento), citando a Nota de Posicionamento sobre IA (jul/2025). **Revogação (LGPD art. 18):** `POST /consentimentos/{id}/revogar` marca `revogado_em` (append-only) e todo gate passa a exigir consentimento **ativo** (não revogado). Migração 014 libera `uso_ia` no CHECK; migração 017 adiciona `revogado_em`.
