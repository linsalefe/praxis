...
- **Conformidade IA-CFP (Res. CFP 09/2024)** — transparência operacionalizada, reusando o que já existe: (1) **TCLE de uso de IA** versionado, registrável via `consentimentos` (`tipo="uso_ia"`, o texto versionado fica em `texto_aceito`); (2) **log de uso de IA por paciente** derivado do `audit_log` (eventos reais e factuais de Sofia/Scribe/Preparação/Instrumento, marcados com `meta.paciente_id`) em `GET /pacientes/{id}/ia-log`, **incluído no pacote LGPD** (`export.json` + `resumo.pdf`); (3) página **pública** `/como-usamos-ia` explicando, de forma clara e verdadeira, o que a IA faz (apoio/rascunho, supervisão humana) e o que **não** faz (não decide, não substitui julgamento), citando a Res. CFP 09/2024 e a Nota de Posicionamento sobre IA (jul/2025). Sem tabela nova; migração 014 apenas libera `uso_ia` no CHECK de `consentimentos`.
